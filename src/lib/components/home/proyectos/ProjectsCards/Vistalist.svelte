<script lang="ts">
    type Props = {
        showGifVistalist?: boolean;
        currentYear?: number;
    };
    let { showGifVistalist = $bindable(false), currentYear }: Props = $props();
</script>

<div class="group relative">
    <div class="absolute -inset-1 rounded-3xl bg-gradient-to-r from-cyan-500 to-blue-600 blur opacity-20 transition duration-1000 group-hover:opacity-60 group-hover:duration-200"></div>

    <a
        href="https://vitalist.vercel.app/"
        rel="noopener noreferrer"
        target="_blank"
        onmouseenter={() => (showGifVistalist = true)}
        onmouseleave={() => (showGifVistalist = false)}
    ...
    >
        <div class="flex h-full flex-col rounded-3xl border border-slate-700/50 bg-slate-800/80 p-6 backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:border-cyan-500/50 sm:p-8">
            ...
            <div class="relative mb-6 h-48 flex-shrink-0 overflow-hidden rounded-2xl bg-slate-950">
                <img
                    src="/gif/vitalist-preview.gif"
                    class="h-full w-full object-cover md:hidden"
                    alt="Vista previa del proyecto Vitalist"
                />
                {#if showGifVistalist}
                    <img
                        src="/gif/vitalist-preview.gif"
                ...
                {:else}
                    <img
                        src="/images/avif_vitalistIcon.avif"
            ...
            <div class="mb-3 flex items-start justify-between gap-3">
                <h3 class="text-2xl font-bold leading-tight text-white transition-colors group-hover:text-cyan-400">
                    Vitalist
                </h3>
                <div class="mt-1 flex-shrink-0 text-slate-500 transition-colors group-hover:text-cyan-400">
            ...
            <p class="mb-4 flex-1 text-sm leading-relaxed text-slate-400">
                Plataforma de gestión de rutinas diarias con seguimiento de hábitos, autenticación segura y almacenamiento en la nube para sincronizar tus tareas en cualquier dispositivo
            </p>
